  ...
  (function($) {
    ...
    if (!Modernizr.touch) {
      ...
      pinScene12Tl
        .fromTo(
          ...
          { y: '+200' },
          {
            ...
            onStart: function() {
              $('.currentArtist')
                .removeClass('currentArtist')
                .removeClass('artistColor');
              $('#exHex')
                .addClass('artistColor')
                .addClass('currentArtist');
      ...
      pinScene13Tl
        ...
        .from(
          $('#slide12 #poloroid-hex'),
          1,
          { autoAlpha: 0, x: -200, ease: Power2.easeIn },
          '-=1'
        )
        ...
        .to(
          $converseShoes,
          1,
          { marginTop: '-5%', ease: Power2.easeOut },
          '-=2'
        )
        .to(
          $('#slide12 #academy-records'),
          1,
          {
            autoAlpha: 1,
            ease: Power1.easeOut,
        ...
        .to(
          $('#slide12 #ex-fade-text'),
          1,
          { autoAlpha: 1, ease: Power1.easeOut },
          '-=0.75'
        )
        .to(
          $('#slide12 #ex-fade-text p'),
          1,
          { y: '-=400', ease: Power1.easeOut },
          '+=0.75'
        );
      ...
      pinScene14Tl
        ...
        ) //.set($('#slide03 header'), { autoAlpha: 1 }, '+=2.5');
      ...
      pinScene15Tl
        ...
        .fromTo(
          ...
          1,
          { y: '+=200' },
          { y: '100', autoAlpha: 1, ease: Power1.easeOut },
          '-=0.2'
        )
        ...
        .to(
          $('#slide14 .timestamp'),
          0.5,
          { color: '#000', ease: Power1.easeIn },
          '-=1.7'
        )
        .to(
          $('#slide14 #baby-triangle'),
          1,
          { top: '-75%', ease: Power1.easeIn },
          '-=2'
        );
      ...
      pinScene16Tl
        ...
        .fromTo(
          ...
          0.5,
          { y: '+=20' },
          { y: '-=50', autoAlpha: 1, ease: Power1.easeOut }
        )
    ...
    var visualize = function() {
      ...
      if (!v) {
        ...
        $('#playingCircles').append(
          $(
            '<div class="artistCircles" id = "' +
              currentSongCircles +
              '"></div>'
          )
        );
        visualizers[$currentSong.attr('id')] = new Visualization({
          song: $currentSong[0],
          renderer: circleRender
        });
        v = visualizers[$currentSong.attr('id')];
      }
      $('.artistCircles')
        .not('#' + currentSongCircles)
        .hide();
...
function playingCircles() {
  ...
  var init = function(config) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      if (i < count / 2) {
        r = Math.min(
          Math.max(Math.round(colorRgb[0] * (factor * Math.max(i / 2, 1))), 0),
          255
        );
        g = Math.min(
          Math.max(Math.round(colorRgb[1] * (factor * Math.max(i / 2, 1))), 0),
          255
        );
        b = Math.min(
          Math.max(Math.round(colorRgb[2] * (factor * Math.max(i / 2, 1))), 0),
          255
        );
      } else {
        r = Math.min(
          Math.max(
            Math.round(
              255 - (255 - colorRgb[0]) * (factor * Math.max(i / 2, 1))
            ),
            0
          ),
          255
        ...
        g = Math.min(
          Math.max(
            Math.round(
              255 - (255 - colorRgb[1]) * (factor * Math.max(i / 2, 1))
            ),
            0
          ),
          255
        ...
        b = Math.min(
          Math.max(
            Math.round(
              255 - (255 - colorRgb[2]) * (factor * Math.max(i / 2, 1))
            ),
            0
          ),
          255
