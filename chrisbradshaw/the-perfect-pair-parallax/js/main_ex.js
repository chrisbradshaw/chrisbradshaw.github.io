...
$(function($) { // Init ScrollMagic
    ...
    // get all slides
    var slides = ["#slide11", "#slide12", "#slide13", "#slide14", "#slide15", "#slide06", "#slide07", "#slide08", "#slide09", "#slide10", "#slide01", "#slide02", "#slide03", "#slide04", "#slide05"];

    // get all headers in slides that trigger animation
    var headers = ["#intro header", "#slide11 header", "#slide12 header", "#slide13 header", "#slide14 header", "#slide15 header", "#slide06 header", "#slide07 header", "#slide08 header", "#slide09 header", "#slide10 header", "#slide01 header", "#slide02 header", "#slide03 header", "#slide04 header", "#slide05 header"];

    // get all break up sections
    var breakSections = ["#cb01", "#cb12", "#cb13", "#cb14", "#cb015", "#cb16", "#cb17", "#cb07", "#cb08", "#cb09", "#cb010", "#cb11", "#cb12", "#cb02", "#cb02", "#cb03", "#cb04", "#cb05", "#cb06"];
    ...
    var noVisualize = !(typeof AudioContext === 'function') //|| typeof webkitAudioContext === 'object');
    ...
    var artistColors = { nickCatchdubs: '#d5ff45', whiteLung: '#ff004e', exHex: '#705edd' };

    window.currentColor = artistColors.exHex;

    var $currentSong = $('#exHexSong');
    ...
    $('#shop-now-ex').hover(function() {
            $('#shop-img-ex')[0].src = path + 'img/ShopNow_Hover_ExHex.png';
        },
        function() {
            $('#shop-img-ex')[0].src = path + 'img/ShopNowLockup_hex.png';
        });
    ...
    if (!Modernizr.touch) {
        ...
        function timeTo(timestamp) {
            ...
            var fromTime = $timestamp.data('fromtime').split(':');
            var toTime = $timestamp.data('totime').split(':');
            var fromTimeHr = parseFloat(fromTime[0]);
            var fromTimeMin = parseFloat(fromTime[1]);
            var toTimeHr = parseFloat(toTime[0]);
            var toTimeMin = parseFloat(toTime[1]);
            var fromTotTime = fromTimeHr == 12 ? 0 : fromTimeHr * 60 + fromTimeMin;
            var toTotTime = toTimeHr * 60 + toTimeMin;
            //console.log("Timestamp: ", $timestamp);
            //console.log("From Time Hr: " + fromTimeHr + " From Time Min: " + fromTimeMin);
            //console.log("To Time Hr: " + toTimeMin + " To Time Min: " + toTimeMin);
            //console.log("From Time: " + fromTotTime + " To Time: " + toTotTime);
            var timeObj = { totTime: fromTotTime };
            var $timestampHr = $timestamp.find('.hr');
            var $timestampMin = $timestamp.find('.min');
            var $meridiem = $timestamp.find('.meridiem');
            var meridiemTo = $timestamp.data('meridiem');
            //console.log($meridiem);
            TweenMax.to(timeObj, 2, {
                totTime: toTotTime,
                roundProps: "totTime",
                onStart: function() {
                    if ($meridiem.length) {
                ...
                onUpdate: function() {
                    ...
                    var toTimeHrUpdate = Math.trunc(timeObj.totTime / 60);
                    var toTimeMinUpdate = timeObj.totTime % 60;
                    //console.log("To time Hr: " + toTimeHrUpdate);
                    //console.log("To Time Min: " + toTimeMinUpdate);
                    $timestampHr.text(toTimeHrUpdate == '0' ? "12" : toTimeHrUpdate);
                    $timestampMin.text(("0" + toTimeMinUpdate).slice(-2));
                },
                ease: Expo.easeOut
            });
        }
        ...
        var pinScene12Tl = new TimelineMax();
        pinScene12Tl
            .fromTo($('#slide11 section'), 1, { y: '+200' }, {
                y: '0',
                onStart: function() {
                    $('.currentArtist').removeClass('currentArtist').removeClass('artistColor');
                    $('#exHex').addClass('artistColor').addClass('currentArtist');
                    $('#nowPlaying')[0].innerHTML = exHexTrack;
                    //switchSongs('whiteLung');
                    var $timestamp = $('#slide11 .timestamp');
                    timeTo($timestamp);
                }
            }, '+=0')
            .to($('#slide11 .timestamp'), 1, { autoAlpha: 1, ease: Power1.easeOut })
            .fromTo($('#slide11 #ex-hex'), 0.5, { y: '+1000', ease: Power1.easeOut }, { y: '0' }, '-=1')
            .set({}, {}, 3); //.to($('#slide01 .videoWrap'), 1, { y: '-100', ease: Power1.easeOut });


        var pinScene12 = new ScrollMagic.Scene({
                triggerElement: '#slide11',
                triggerHook: 0,
                duration: '200%'
            })
            .setPin('#slide11')
            .setTween(pinScene12Tl)
            .addTo(controller);

        // Ex Hex SLIDE 2: -->

        var $vitusTimestamp = $('#slide12 #union-pool');
        //var $comicQuote = $('#slide07 .comic-quote');
        var $converseShoes = $('#slide12 #ex-converse-shoes');
        var $s12bcg = $('#slide12 .bcg');
        var $s12overbcg = $('#slide12 .overbcg');

        var pinScene13Tl = new TimelineMax();
        pinScene13Tl
            .fromTo($vitusTimestamp, 1, { y: '+=20' }, {
                autoAlpha: 1,
                ease: Power1.easeOut,
                onStart: function() {
                    var $timestamp = $('#slide12 #union-pool.timestamp');
                    timeTo($timestamp);
                }
            })
            .from($('#slide12 #poloroid-hex'), 1, { autoAlpha: 0, x: -200, ease: Power2.easeIn }, '-=1')
            //.to($comicQuote, 1, { autoAlpha: 1, ease: Power1.easeOut })
            .to($vitusTimestamp, 1, { autoAlpha: 0, ease: Power1.easeOut }, '+=0.5')
            //.to($s7bcg, 1.5, { marginTop: '-75%', ease: Power1.easeOut }, '-=0.5')
            .to($s12overbcg, 1.5, { top: 0, ease: Power1.easeOut }, '-=0.5')
            .to($('#slide12 #poloroid-hex'), 1, { autoAlpha: 0 }, '-=1.5')
            .to($converseShoes, 1, { marginTop: '-5%', ease: Power2.easeOut }, '-=2')
            .to($('#slide12 #academy-records'), 1, {
                autoAlpha: 1,
                ease: Power1.easeOut,
                onStart: function() {
                    var $timestamp = $('#slide12 #academy-records.timestamp');
                    timeTo($timestamp);
                }
            }, '-=1')
            .to($('#slide12 #ex-fade-text'), 1, { autoAlpha: 1, ease: Power1.easeOut }, '-=0.75')
            .to($('#slide12 #ex-fade-text p'), 1, { y: '-=400', ease: Power1.easeOut }, '+=0.75');

        var pinScene13 = new ScrollMagic.Scene({
                triggerElement: '#slide12',
                triggerHook: 0,
                duration: '500%'
            })
            .setPin('#slide12')
            .setTween(pinScene13Tl)
            .addTo(controller);

        // <!-- SLIDE 3 CONVERSE SNEAKERS -->

        var pinScene14Tl = new TimelineMax();
        pinScene14Tl
            .fromTo($('#slide13 header'), 1, { y: '+=20' }, {
                y: 0,
                autoAlpha: 1,
                ease: Power1.easeOut,
                onStart: function() {
                    var $timestamp = $('#slide13 .timestamp');
                    timeTo($timestamp);
                }
            })
            .fromTo($('#slide13 section'), 1, { y: '+=20' }, { y: 0, autoAlpha: 1, ease: Power1.easeOut }) //.set($('#slide03 header'), { autoAlpha: 1 }, '+=2.5');
            .to($('#slide13 section'), 1, { autoAlpha: 1 });
        var pinScene14 = new ScrollMagic.Scene({
                triggerElement: '#slide13',
                triggerHook: 0,
                duration: '150%'
            })
            .setPin('#slide13')
            .setTween(pinScene14Tl)
            .addTo(controller);

        // White Lung SLIDE 4: East River

        var pinScene15Tl = new TimelineMax();
        pinScene15Tl
            .to($('#slide14'), 0.4, {
                autoAlpha: 1,
                onStart: function() {
                    var $timestamp = $('#slide14 .timestamp');
                    timeTo($timestamp);
                }
            })
            .fromTo($('#slide14 header'), 0.7, { y: '+=20' }, { y: 0, autoAlpha: 1, ease: Power1.easeOut })
            .to($('#slide14 #group-photo'), 1, { top: '185px', ease: Power1.easeOut })
            //.to($('#slide04 #pinball-triangle'), 1, { top: '-15%', ease: Power1.easeOut }, '-=2')
            .fromTo($('#slide14 section'), 1, { y: '+=200' }, { y: '100', autoAlpha: 1, ease: Power1.easeOut }, '-=0.2')
            .to($('#slide14 section'), 1, { y: '-300px', ease: Power1.easeOut })
            .to($('#slide14 .timestamp'), 0.5, { color: '#000', ease: Power1.easeIn }, '-=1.7')
            .to($('#slide14 #baby-triangle'), 1, { top: '-75%', ease: Power1.easeIn }, '-=2');

        var pinScene15 = new ScrollMagic.Scene({
            ...
            .setPin('#slide14')
            .setTween(pinScene15Tl)
            .addTo(controller);

        // White Lung SLIDE 5: East Village

        var pinScene16Tl = new TimelineMax();
        pinScene16Tl
            .fromTo($('#slide15 header'), 0.7, { y: '+=20' }, {
                y: 0,
                autoAlpha: 1,
                ease: Power1.easeOut,
                onStart: function() {
                    var $timestamp = $('#slide15 .timestamp');
                    timeTo($timestamp);
                }
            })
            .fromTo($('#slide15 section'), 0.5, { y: '+=20' }, { y: '-=50', autoAlpha: 1, ease: Power1.easeOut })
            .to($('#slide15 section'), 1, {
                y: '-=400',
                ease: Power1.easeOut
            }); //.to($('#slide05 section'), 0.5, { y: '-100px', ease: Power1.easeOut }); //.set($('#slide05 header'), { autoAlpha: 1 }, '+=2.5');

        var pinScene16 = new ScrollMagic.Scene({
        ...
        pinScene07Tl
            .fromTo($('#slide06 section'), 1, { y: '+200' }, {
                ...
                onStart: function() {
                    ...
                    switchSongs('whiteLung');
                    var $timestamp = $('#slide06 .timestamp');
                    timeTo($timestamp);
                }
            }, '+=0')
            .to($('#slide06 .timestamp'), 1, { autoAlpha: 1, ease: Power1.easeOut })
            .fromTo($('#slide06 #white-lung'), 0.5, { y: '+1000', ease: Power1.easeOut }, { y: '0' }, '-=1')
            .set({}, {}, 3); //.to($('#slide01 .videoWrap'), 1, { y: '-100', ease: Power1.easeOut });
        ...
        pinScene08Tl
            .fromTo($vitusTimestamp, 1, { y: '+=20' }, {
                ...
                ease: Power1.easeOut,
                onStart: function() {
                    var $timestamp = $('#slide07 #st-vitus.timestamp');
                    timeTo($timestamp);
                }
            })
            //.to($comicQuote, 1, { autoAlpha: 1, ease: Power1.easeOut })
            ...
            .to($converseShoes, 1, { marginTop: '-5%', ease: Power2.easeOut }, '-=2')
            .to($('#slide07 #milk-honey'), 1, {
                autoAlpha: 1,
                ease: Power1.easeOut,
                onStart: function() {
                    var $timestamp = $('#slide07 #milk-honey.timestamp');
                    timeTo($timestamp);
                }
            }, '-=1')
        ...
        pinScene09Tl
            .fromTo($('#slide08 header'), 1, { y: '+=20' }, {
                y: 0,
                autoAlpha: 1,
                ease: Power1.easeOut,
                onStart: function() {
                    var $timestamp = $('#slide08 .timestamp');
                    timeTo($timestamp);
                }
            })
        ...
        pinScene10Tl
            .to($('#slide09'), 0.4, {
                autoAlpha: 1,
                onStart: function() {
                    var $timestamp = $('#slide09 .timestamp');
                    timeTo($timestamp);
                }
            })
        ...
        pinScene11Tl
            .fromTo($('#slide10 header'), 0.7, { y: '+=20' }, {
                y: 0,
                autoAlpha: 1,
                ease: Power1.easeOut,
                onStart: function() {
                    var $timestamp = $('#slide10 .timestamp');
                    timeTo($timestamp);
                }
            })
        ...
        pinScene02Tl
            .fromTo($('#slide01 section'), 1, { y: '+200' }, {
                ...
                onStart: function() {
                    ...
                    var $timestamp = $('#slide01 .timestamp');
                    timeTo($timestamp);
                }
            }, '+=0')
            .to($('#slide01 .timestamp'), 1, { autoAlpha: 1, ease: Power1.easeOut })
            .fromTo($('#slide01 #nick-catchdubs'), 0.5, { y: '+1000', ease: Power1.easeOut }, { y: '0' }, '-=1')
            .set({}, {}, 3); //.to($('#slide01 .videoWrap'), 1, { y: '-100', ease: Power1.easeOut });
        ...
        pinScene03Tl
            .fromTo($comicTimestamp, 1, { y: '+=20' }, {
                autoAlpha: 1,
                ease: Power1.easeOut,
                onStart: function() {
                    var $timestamp = $('#slide02 #comic-book.timestamp');
                    timeTo($timestamp);
                }
            })
            .to($comicQuote, 1, { autoAlpha: 1, ease: Power1.easeOut })
            ...
            .to($('#slide02 #gold-store'), 1, {
                autoAlpha: 1,
                ease: Power1.easeOut,
                onStart: function() {
                    var $timestamp = $('#slide02 #gold-store.timestamp');
                    timeTo($timestamp);
                }
            }, '-=1')
        ...
        pinScene04Tl
            .fromTo($('#slide03 header'), 1, { y: '+=20' }, {
                y: 0,
                autoAlpha: 1,
                ease: Power1.easeOut,
                onStart: function() {
                    var $timestamp = $('#slide03 .timestamp');
                    timeTo($timestamp);
                }
            })
        ...
        pinScene05Tl
            .to($('#slide04'), 0.4, {
                autoAlpha: 1,
                onStart: function() {
                    var $timestamp = $('#slide04 .timestamp');
                    timeTo($timestamp);
                }
            })
        ...
        pinScene06Tl
            .fromTo($('#slide05 header'), 0.7, { y: '+=20' }, {
                y: 0,
                autoAlpha: 1,
                ease: Power1.easeOut,
                onStart: function() {
                    var $timestamp = $('#slide05 .timestamp');
                    timeTo($timestamp);
                }
            })
    ...
    var visualize = function() {
        ...
        //console.log(v);
        var currentSongCircles = $currentSong.attr('id') + 'Circles';
        if (!v) {
            //console.log("new visualizer");
            $('#playingCircles').append($('<div class="artistCircles" id = "' + currentSongCircles + '"></div>'));
            visualizers[$currentSong.attr('id')] = new Visualization({ song: $currentSong[0], renderer: circleRender });
            v = visualizers[$currentSong.attr('id')];
        }
        $('.artistCircles').not('#' + currentSongCircles).hide();
        $('#' + currentSongCircles).show();
    ...
    function switchSongs(artist) {
        ...
        window.currentColor = artistColors[artist];
...
function Visualization(config) {
    ...
    var init = function(_song) {
        ...
        renderer.init({ song: $(song).attr('id'), count: analyser.frequencyBinCount, width: width, height: height });
    };
    this.start = function() {
    ...
    this.setRenderer = function(r) {
        if (!r.isInitialized()) { r.init({ song: $(config.song).attr('id'), count: analyser.frequencyBinCount, width: width, height: height }); }
        renderer = r;
    };
...
function playingCircles() {
    ...
    var colorRgb = [parseInt(color.substring(1, 3), 16),
    ...
    var init = function(config) {
        ...
        var circlesEl = document.getElementById(config.song + 'Circles');
        //console.log("song: " + config.song, circlesEl);
        var r, g, b;
        for (var i = 0; i < count; i++) {
            ...
            if (i < count / 2) {
                r = Math.min(Math.max(Math.round(colorRgb[0] * (factor * Math.max(i / 2, 1))), 0), 255);
                g = Math.min(Math.max(Math.round(colorRgb[1] * (factor * Math.max(i / 2, 1))), 0), 255);
                b = Math.min(Math.max(Math.round(colorRgb[2] * (factor * Math.max(i / 2, 1))), 0), 255);
            } else {
                r = Math.min(Math.max(Math.round(255 - (255 - colorRgb[0]) * (factor * Math.max(i / 2, 1))), 0), 255);
                g = Math.min(Math.max(Math.round(255 - (255 - colorRgb[1]) * (factor * Math.max(i / 2, 1))), 0), 255);
                b = Math.min(Math.max(Math.round(255 - (255 - colorRgb[2]) * (factor * Math.max(i / 2, 1))), 0), 255);
            }
            //console.log("r: " + r + " g: " + g + " b: " + b);
            colorArray.push('rgb(' + r + ',' + g + ',' + b + ')'); //rgbToHex(r, g, b));
    ...
    var renderFrame = function(frequencyData) {
        for (var i = 0; i < circles.length; i++) {
            ...
            circle.style.cssText = '-webkit-transform:scale(' + ((frequencyData[i] / max)) + '); box-shadow: 0 0 2px 2px ' + colorArray[i] + ';';
        }
    };
    return {
        init: init,
